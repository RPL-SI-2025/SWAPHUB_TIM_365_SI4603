          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead>
              <tr class="bg-gradient-to-r from-blue-900 to-blue-800">
                <th
                  class="px-4 py-4 text-left text-xs font-medium text-white uppercase tracking-wider w-1/6 first:rounded-tl-3xl">
                  Penawar</th>
                <th class="px-4 py-4 text-left text-xs font-medium text-white uppercase tracking-wider w-1/6">Ditawar
                </th>
                <th class="px-4 py-4 text-left text-xs font-medium text-white uppercase tracking-wider w-1/6">Tanggal
                </th>
                <th class="px-4 py-4 text-left text-xs font-medium text-white uppercase tracking-wider w-1/4">Riwayat
                  Penukaran</th>
                <th class="px-4 py-4 text-left text-xs font-medium text-white uppercase tracking-wider w-1/6">Kategori
                </th>
                <th class="px-4 py-4 text-left text-xs font-medium text-white uppercase tracking-wider w-1/12">Status
                </th>
                <th
                  class="px-4 py-4 text-left text-xs font-medium text-white uppercase tracking-wider w-1/12 last:rounded-tr-3xl">
                  Aksi</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              @forelse($histories as $history)
                <tr class="hover:bg-blue-50/50 transition-colors duration-200">
                  <td class="px-4 py-4">
                    <div class="flex items-center">
                      <div class="flex-shrink-0 h-8 w-8 bg-blue-100 rounded-full flex items-center justify-center">
                        <span
                          class="text-blue-600 font-medium">{{ substr($history->penukaran->penawar->first_name ?? 'U', 0, 1) }}</span>
                      </div>
                      <div class="ml-3">
                  ...
                  <td class="px-4 py-4">
                    <div class="flex items-center">
                      <div class="flex-shrink-0 h-8 w-8 bg-gray-100 rounded-full flex items-center justify-center">
                        <span
                          class="text-gray-600 font-medium">{{ substr($history->penukaran->ditawar->first_name ?? 'U', 0, 1) }}</span>
                      </div>
                      <div class="ml-3">
                        <div class="text-sm font-medium text-gray-900 truncate max-w-[120px]">
                  ...
                  <td class="px-4 py-4">
                    <div class="text-sm">
                      <p class="font-medium text-gray-900 mb-1 truncate max-w-[200px]"
                        title="{{ $history->penukaran->barangPenawar->nama_barang ?? '-' }}">
                        {{ $history->penukaran->barangPenawar->nama_barang ?? '-' }}
                      </p>
                      <p class="text-gray-500 flex items-center truncate max-w-[200px]"
                        title="{{ $history->penukaran->barangDitawar->nama_barang ?? '-' }}">
                  ...
                  <td class="px-4 py-4">
                    <div class="text-sm">
                      <p class="text-gray-900 truncate max-w-[150px]"
                        title="{{ $history->penukaran->barangPenawar->kategori->nama_kategori ?? '-' }}">
                        {{ $history->penukaran->barangPenawar->kategori->nama_kategori ?? '-' }}
                      </p>
                      <p class="text-gray-500 truncate max-w-[150px]"
                        title="{{ $history->penukaran->barangDitawar->kategori->nama_kategori ?? '-' }}">
                  ...
                  <td class="px-4 py-4">
                    <span
                      class="px-3 py-1.5 text-sm font-medium rounded-full inline-flex items-center
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        @foreach ($histories as $history)
          ...
          @if (
            ...
            <div
              class="bg-white rounded-3xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
              <div class="mb-4">
                <h3 class="text-sm font-medium text-gray-900 line-clamp-2">
                  {{ $history->penukaran->barangPenawar->nama_barang }} dengan
                  {{ $history->penukaran->barangDitawar->nama_barang }}
                </h3>
              ...
              @if ($userRating)
                <div class="flex items-center justify-between">
                  <span
                    class="inline-flex items-center px-3 py-1.5 bg-gray-100 text-gray-800 rounded-xl text-xs font-medium">
