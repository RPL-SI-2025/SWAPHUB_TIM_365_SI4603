...
<div class="container mx-auto mt-8">
    ...
    <form action="{{ route('admin.rekomendasi.index') }}" method="GET" class="mb-6 space-y-4">
        ...
        <div>
            ...
            <select name="category_filter" id="category_filter" class="border rounded p-2 w-full" onchange="this.form.submit()">
                <option value="">-- Semua Kategori --</option>
                @foreach ($kategori as $ktg)
                    <option value="{{ $ktg->id_kategori }}" {{ $selectedCategoryFilter == $ktg->id_kategori ? 'selected' : '' }}>
                        {{ $ktg->nama_kategori }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <div class="mb-2">
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" id="checkAll" class="form-checkbox rounded border-gray-300 focus:ring focus:ring-blue-300" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 max-h-80 overflow-auto border border-gray-300 rounded p-3">
                ...
                <label class="inline-flex items-center space-x-2 p-2 border rounded cursor-pointer hover:bg-gray-50">
                    <input type="checkbox" name="barang_ids[]" value="{{ $barang->id_barang }}" class="item-checkbox rounded border-gray-300 focus:ring focus:ring-blue-300">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50 transition">
                        ...
                        <td class="py-3 px-4 whitespace-nowrap">
                            <form action="{{ route('admin.rekomendasi.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus rekomendasi ini?')" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <table class="min-w-full table-auto bg-white border border-gray-300 rounded-lg shadow overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-blue-50 transition">
                    ...
                    <td class="py-3 px-4 whitespace-nowrap">
                        {{ $history->penukaran->barangPenawar->nama_barang ?? '-' }} ↔ {{ $history->penukaran->barangDitawar->nama_barang ?? '-' }}
                    </td>
                    <td class="py-3 px-4 whitespace-nowrap">
