...
<body class="bg-gray-100">
    <div class="container mx-auto p-6">
        ...
        @if ($penukaranMasuk->isEmpty())
            <p class="text-gray-600">Belum ada permintaan tukar barang.</p>
        @else
            <table class="w-full bg-white shadow-md rounded">
                ...
                <tbody>
                    @foreach ($penukaranMasuk as $penukaran)
                        <tr>
                            <td class="p-3">{{ $penukaran->barang->nama_barang }}</td>
                            <td class="p-3">{{ $penukaran->barangDitawarkan->nama_barang }}</td>
                            <td class="p-3">{{ $penukaran->requester->name }}</td>
                            <td class="p-3">{{ $penukaran->riwayat_penukaran ?? '-' }}</td>
