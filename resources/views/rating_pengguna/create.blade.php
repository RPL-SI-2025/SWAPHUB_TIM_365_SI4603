    ...
    <div class="px-4 md:px-24">
        <div class="max-w-4xl mx-auto space-y-8">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-fade-in">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-3xl font-bold">
                        Tambah <span class="bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-blue-800">Rating</span>
                        <span class="text-lg font-normal text-gray-600 ml-2">
                            @if($rating_type === 'penawar')
                                (Sebagai Penawar)
                            @else
                                (Sebagai Yang Ditawar)
                            @endif
                        </span>
                    </h1>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100 transform hover:shadow-xl transition-all duration-300 animate-fade-in-up">
                <form action="{{ route('rating_pengguna.store') }}" method="POST" class="space-y-8">
                    @csrf
                    <input type="hidden" name="id_penukaran_barang" value="{{ request('id_penukaran') }}">
                    <input type="hidden" name="rating_type" value="{{ $rating_type }}">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative group">
                            <textarea 
                                name="review" 
                                class="w-full px-5 py-4 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 min-h-[180px] resize-none group-hover:border-blue-300"
                                placeholder="Bagaimana pengalaman penukaran Anda?"
                                required></textarea>
